...
const glyphColors = {
    [GlyphKind.Lock]: ["#f0b232", "STATUS_WARNING"],
    [GlyphKind.Warning]: ["#f23f43", "STATUS_DANGER"],
    [GlyphKind.Controller]: ["#23a55a", "STATUS_POSITIVE"],
} satisfies Record<GlyphKind, [fallback: string, semanticTokenName: string]>;

const glyphs = [
    { name: "Announcements", kind: GlyphKind.Lock, base: announcementLock },
    { name: "Announcements", kind: GlyphKind.Warning, base: announcementWarning },
    { name: "Text", kind: GlyphKind.Lock, base: textLock },
    { name: "Text", kind: GlyphKind.Warning, base: textWarning },
    { name: "Text", kind: GlyphKind.Controller, base: textController },
    { name: "Voice", kind: GlyphKind.Lock, base: voiceLock },
    { name: "Voice", kind: GlyphKind.Warning, base: voiceWarning },
    { name: "Forum", kind: GlyphKind.Lock, base: forumLock },
    { name: "Forum", kind: GlyphKind.Warning, base: forumWarning },
    { name: "Stage", kind: GlyphKind.Lock, bottom: true, base: stageLock },
    { name: "Stage", kind: GlyphKind.Warning, bottom: true, base: 0 },
    { name: "Image", kind: GlyphKind.Lock, bottom: true, base: imageLock },
    { name: "Image", kind: GlyphKind.Warning, bottom: true, base: imageWarning },
];

const ChannelInfo = findByName("ChannelInfo", false);
const BaseChannelIconModule = findByProps("BaseChannelIcon");
...
export default new Module({
    ...
    handlers: {
        onStart() {
            if (this.storage.options.nsfwTag && ChannelInfo) {
                this.patches.add(
                    after("default", ChannelInfo, ([{ channel }]: any[], ret: any) =>
                        React.createElement(
                            React.Fragment,
                            {},
                            channel?.nsfw_ && React.createElement(TextBadge, { variant: "destructive" }, "nsfw"),
                            ret,
                        )),
                );
            }

            if (this.storage.options.colorIcons && BaseChannelIconModule?.BaseChannelIcon) {
                this.patches.add(
                    after("BaseChannelIcon", BaseChannelIconModule, ([{ source, style }]: any[], ret: any) => {
                        try {
                            ...
                            const [fallbackColor, tokenName] = glyphColors[glyph.kind];
                            const overlay = getGlyphOverlay(glyph.kind, glyph.bottom);

                            const iconColorKey = ret?.props?.color?.replace(/-/g, "_")?.toUpperCase();
                            const iconTint = iconColorKey && (semanticColors as Record<string, any>)?.[iconColorKey]
                                ? resolveSemanticColor((semanticColors as Record<string, any>)[iconColorKey])
                                : undefined;

                            const styled = RN.StyleSheet.flatten([
                                style,
                                ret?.props?.style,
                                { width: 18, height: 18, tintColor: iconTint },
                            ]);

                            const overlayTint = this.storage.options.colorIconsFallback
                                ? fallbackColor
                                : resolveSemanticColor((semanticColors as Record<string, any>)?.[tokenName]) ?? fallbackColor;

                            return React.createElement(
                                RN.View,
                                {},
                                React.createElement(RN.Image, { style: styled, source: glyph.base }),
                                React.createElement(
                                    RN.View,
                                    { style: { position: "absolute", right: 0, bottom: 0 } },
                                    React.createElement(RN.Image, { style: { ...styled, tintColor: overlayTint }, source: overlay }),
                                ),
                            );
                        } catch {
                            // Leave the channel icon as Discord rendered it.
                            return undefined;
                        }
                    }),
                );
